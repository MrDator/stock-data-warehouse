...
def sanitize_beta(raw_beta, sector_type):
    """修复 Beta 值 (如 PDD 0.05 -> 1.3)"""
    if raw_beta is None: return 1.0
    if raw_beta < 0.5:
        if sector_type in ['SaaS', 'Semiconductor', 'BioTech']: return 1.3
        else: return 1.0
    if raw_beta > 3.0: return 2.5
    return raw_beta
...
def fetch_stock_data(ticker_symbol):
    try:
        ...
        # Beta 修复
        beta = sanitize_beta(info.get('beta'), sector_type)
        
        # [NEW] 股息率 (Dividend Yield)
        # yf 返回 0.034 代表 3.4%
        ...
        data = {
            ...
            "dividend_yield": round(dividend_yield, 2), # [NEW]
...
def main():
    print("--- Starting Full-Stack Data Pipeline (v3) ---")
